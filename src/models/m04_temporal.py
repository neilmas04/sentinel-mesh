...
def prepare_data(cohort_name: str, labels_df: pd.DataFrame) -> pd.DataFrame:
    ...
    df = temporal_extractor.extract_features(df)
...
def run_experiment():
    ...
    ablations = {
        ...
        'System B (All Network)': LOCAL_FEATURES + ALL_NETWORK_FEATURES,
        'System B* (Local + Device + Account)': LOCAL_FEATURES + B_STAR_FEATURES,
        'System C (B* + Rolling)': LOCAL_FEATURES + B_STAR_FEATURES + ROLLING_FEATURES,
        'System C (B* + Growth)': LOCAL_FEATURES + B_STAR_FEATURES + GROWTH_FEATURES,
        'System C (B* + Synchronization)': LOCAL_FEATURES + B_STAR_FEATURES + SYNC_FEATURES,
        'System C (B* + All Temporal)': LOCAL_FEATURES + B_STAR_FEATURES + ALL_TEMPORAL_FEATURES,
        'System C (All Network + All Temporal)': LOCAL_FEATURES + ALL_NETWORK_FEATURES + ALL_TEMPORAL_FEATURES
    }
    ...
    for name, features in ablations.items():
        ...
        if name == 'System C (B* + All Temporal)':
            artifact_dir = "artifacts/experiments/m04_system_c"
            os.makedirs(artifact_dir, exist_ok=True)
            model_path = os.path.join(artifact_dir, "system_c_model.pkl")
